...
namespace MagnumOpus.Squiggly
{
    ...
    //     private static void Process(PixelEntity target, PixelEntity invoker, cq_action cqaction, SquigglyContext db)
    //     {
    ...
    //             default:
    //                 {
    //                     FConsole.WriteLine($"Unknown Cq_ActionId -> {cqaction}", ConsoleColor.Blue);
    //                     break;
    //                 }
    //             case Cq_ActionId.ACTION_MENUTEXT:
    //                 {
    //                     if (invoker is PixelEntity player)
    //                     {
    //                         player.NetSync(LegacyPackets.NpcSay(cqaction.param.Trim().Replace("~", " ")));
