...
public static class ReflectionHelper
{
    ...
    private static void LoadMethods()
    {
        ...
        {
            var removeMethod = typeof(PackedComponentStorage<>).MakeGenericType(ct).GetMethod("Remove")!.CreateDelegate<Action<NTT, bool>>();
            RemoveCache.Add(ct, removeMethod);

            var changeOwnerMethod = (Action<NTT, NTT>)typeof(PackedComponentStorage<>).MakeGenericType(ct).GetMethod("ChangeOwner")!.CreateDelegate(typeof(Action<NTT, NTT>));
            ...
            {
                var saveMethod = typeof(PackedComponentStorage<>).MakeGenericType(ct).GetMethod("Save")!.CreateDelegate<Action<string>>();
                SaveCache.Add(ct, saveMethod);

                var loadMethod = typeof(PackedComponentStorage<>).MakeGenericType(ct).GetMethod("Load")!.CreateDelegate<Action<string>>();
