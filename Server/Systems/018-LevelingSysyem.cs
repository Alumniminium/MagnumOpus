...
namespace MagnumOpus.Systems
{
    public sealed class LevelingSystem : NttSystem<LevelComponent, ExpRewardComponent>
    {
        ...
        public override void Update(in NTT ntt, ref LevelComponent levelComponent, ref ExpRewardComponent experienceReward)
        {
            ...
            levelComponent.Experience += expGained;

            // Check if entity has enough experience to level up
            var canLevelUp = levelComponent.Experience >= levelComponent.ExperienceToNextLevel;
            ...
            levelComponent.ExperienceToNextLevel = (uint)Collections.LevelExps.Values[newLevel - 1];
            ...
            var attributeAllocation = Collections.CqPointAllot.FirstOrDefault(x => x.level == newLevel && x.profession == professionBase);
            ...
            if (IsLogging)
            {
                FConsole.WriteLine("{ntt} gained {exp} exp and leveled to {lvl}, now at {current}/{next}",
                    ntt, expGained, newLevel, levelComponent.Experience,
                    levelComponent.ExperienceToNextLevel);
            }
